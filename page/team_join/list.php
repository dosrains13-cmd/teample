...
<form method="get" id="moveForm" name="moveForm">
    <input type="hidden" name="page" value="<?php echo $page; ?>">
    <input type="hidden" name="<?php echo $parent_key_column; ?>" value="<?php echo $parent_key; ?>" />
    <input type="hidden" name="<?php echo $key_column; ?>" value="" />
    <?php foreach($sweb['list']['qstr'] as $key_qstr => $value_qstr){ ?>
    <input type="hidden" name="<?php echo $key_qstr; ?>" value="<?php echo $value_qstr; ?>">
    <?php } ?>
</form>
...
$(function(){
    ...
    // 더보기 메뉴 이벤트 (이벤트 위임 사용)
    $(document).on('click', '.more button', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        // 모든 다른 floating 메뉴 먼저 닫기
        $('.more .floating').not($(this).siblings('.floating')).removeClass('active');
        
        // 현재 버튼의 floating 메뉴 토글
        $(this).siblings('.floating').toggleClass('active');
    });
    
    // 외부 클릭 시 모든 floating 메뉴 닫기
    $(document).on('click', function(e) {
        if (!$(e.target).closest('.more').length) {
            $('.more .floating').removeClass('active');
        }
    });
});
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (state.page === 1) {
                ...
                var $items = $content.filter('li');
                if ($items.length === 0) {
                    $items = $content.find('li');
                }
                $items.appendTo('#item_area');
            }
...
function fn_delete(key) {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            console.error('탈퇴 처리 중 오류:', textStatus, errorThrown);
            alert('처리 중 오류가 발생했습니다. 다시 시도해주세요.');
        },
        complete: function() {
            if (typeof loadingEnd === 'function') {
...
function fn_joinDelete(key) {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            console.error('팀 탈퇴 처리 중 오류:', textStatus, errorThrown);
            alert('처리 중 오류가 발생했습니다. 다시 시도해주세요.');
        },
        complete: function() {
            if (typeof loadingEnd === 'function') {
